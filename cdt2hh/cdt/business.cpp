...
#define loglevel "us::trader::r2r::cdt2hh/cdt"
#define logclass "business_t"
#include <us/gov/logs.inc>

using namespace us::trader::r2r::cdt2hh;
using c = us::trader::r2r::cdt2hh::cdt::business_t;
...
ko c::init(const string& r2rhome, protocol_factories_t& f) {
    ...
    log("cdt2hh cdt init. homebase", home);
    assert(!home.empty());
    log("cdt2hh cdt initiated @", home);
    return ok;
}

c::protocol_factory_id_t c::protocol_factory_id() const {
    return protocol_factory_id_t(c::protocol::name, "cdt");
}
...
void c::list_protocols(ostream& os) const {
    os << c::protocol::name << " cdt\n";
}

void c::published_protocols(protocols_t& protocols, bool inverse) const {
    protocols.emplace_back(c::protocol::name, inverse ? "hh" : "cdt");
}
